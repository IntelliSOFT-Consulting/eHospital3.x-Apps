...
const PatientInfo: React.FC<{ label: string; value: string }> = ({
  label,
  value,
}) => {
  return (
...
const ConfirmPrompt: React.FC<ConfirmPromptProps> = ({
  ...
  const { facilityName, isLoading } = useFacilityName(
    patient?.originFacilityKmflCode
  );
    ...
    <>
      <div className="cds--modal-header">
        <h3 className="cds--modal-header__heading">
          {t(
            "clientRegistryEmpty",
            `Patient ${patient?.firstName} ${patient?.lastName} found`
          )}
        </h3>
      </div>
        ...
        <div style={{ display: "flex", margin: "1rem" }}>
          ...
          <div style={{ width: "100%", marginLeft: "0.625rem" }}>
            ...
            <PatientInfo
              ...
              value={capitalize(patient?.gender)}
            />
            <PatientInfo
              label={t("nascopNumber", "Nascop facility no")}
              value={capitalize(patient?.nascopCCCNumber)}
            />
            <PatientInfo
