...
const DeleteIdentifierConfirmationModal: React.FC<
  DeleteIdentifierConfirmationModalProps
> = ({ deleteIdentifier, identifierName, identifierValue }) => {
  const { t } = useTranslation();

  return (
    <div className={styles.modalContent}>
      <h1 className={styles.productiveHeading}>
      ...
      <div className={styles.buttonSet}>
        <Button
          kind="secondary"
          size="lg"
          onClick={() => deleteIdentifier(false)}
        >
          {t("cancel", "Cancel")}
        </Button>
