...
const fillRequiredFields = async () => {
  ...
  const demographicsSection = await screen.findByLabelText(
    "Demographics Section"
  );
  const givenNameInput = within(demographicsSection).getByLabelText(
    /first/i
  ) as HTMLInputElement;
  const familyNameInput = within(demographicsSection).getByLabelText(
    /family/i
  ) as HTMLInputElement;
  const dateOfBirthInput = within(demographicsSection).getByLabelText(
    /date of birth/i
  ) as HTMLInputElement;
  const genderInput = within(demographicsSection).getByLabelText(
    /Male/
...
describe("Registering a new patient", () => {
  beforeEach(() => {
    ...
    mockedSavePatient.mockReturnValue({
      data: { uuid: "new-pt-uuid" },
      ok: true,
    });
  ...
  it("should not save the patient if validation fails", async () => {
    ...
    render(
      <PatientRegistration
        isOffline={false}
        savePatientForm={mockedSavePatientForm}
      />,
      { wrapper: Wrapper }
    );

    const givenNameInput = (await screen.findByLabelText(
      "First Name"
    )) as HTMLInputElement;
  ...
  it("renders and saves registration obs", async () => {
    ...
    const weight = within(customSection).getByLabelText(
      "Weight (kg) (optional)"
    );
    await user.type(weight, "50");
    const complaint = within(customSection).getByLabelText(
  ...
  it("retries saving registration obs after a failed attempt", async () => {
    ...
    expect(mockedShowSnackbar).toHaveBeenCalledWith(
      expect.objectContaining({ subtitle: "an error message" })
    ),
      mockedSaveEncounter.mockResolvedValue({});
...
describe("Updating an existing patient record", () => {
  ...
  it("edits patient demographics", async () => {
    ...
    render(
      <PatientRegistration
        isOffline={false}
        savePatientForm={mockedSavePatient}
      />,
      { wrapper: Wrapper }
    );

    const givenNameInput: HTMLInputElement =
      screen.getByLabelText(/First Name/);
    const familyNameInput: HTMLInputElement =
      screen.getByLabelText(/Family Name/);
    const middleNameInput: HTMLInputElement =
      screen.getByLabelText(/Middle Name/);
    const dateOfBirthInput: HTMLInputElement =
