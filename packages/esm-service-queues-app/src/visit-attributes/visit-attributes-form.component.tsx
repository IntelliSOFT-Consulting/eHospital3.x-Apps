...
import { usePaymentMethods } from "./visit-attribute.resource";
...
const VisitAttributesForm: React.FC<VisitAttributesFormProps> = ({
  ...
  const { patientCatergory, catergoryConcepts, nonPayingPatientCategories } =
    useConfig();
  ...
  const { paymentModes, isLoading: isLoadingPaymentModes } =
    usePaymentMethods();

  const patientCategoryOptions = React.useMemo(() => {
    return Object.entries(nonPayingPatientCategories ?? {}).map(
      ([key, uuid]) => ({
        text: key,
        uuid,
      })
  ...
  const createVisitAttributesPayload = () => {
    ...
    return visitAttributesPayload.map((item) => ({
      attributeType: item.uuid,
      value: item.value,
    }));
  };
  ...
  return (
    <section>
      ...
      <Controller
        ...
        render={({ field }) => (
          ...
          >
            <RadioButton
              labelText="Paying"
              value={catergoryConcepts.payingDetails}
              id="radio-1"
            />
            <RadioButton
              labelText="Non paying"
              value={catergoryConcepts.nonPayingDetails}
              id="radio-2"
            />
          </RadioButtonGroup>
        )}
      ...
      {paymentMethods === catergoryConcepts.insuranceDetails &&
        paymentDetails === catergoryConcepts.payingDetails && (
          <>
            <Controller
              control={control}
