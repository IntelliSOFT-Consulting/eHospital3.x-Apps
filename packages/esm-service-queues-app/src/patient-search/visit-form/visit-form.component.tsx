...
const VisitForm: React.FC<VisitFormProps> = ({
  ...
  const [contentSwitcherIndex, setContentSwitcherIndex] = useState(
  ...
  const { activePatientEnrollment, isLoading } =
    useActivePatientEnrollment(patientUuid);
  const [enrollment, setEnrollment] = useState<PatientProgram>(
    activePatientEnrollment[0]
  );
        ...
        <div className={styles.backButton}>
          {mode === true ? null : (
            <Button
              ...
              iconDescription={t(
                "backToScheduledVisits",
                "Back to scheduled visits"
              )}
              size="sm"
              onClick={() =>
                toggleSearchType(SearchTypes.SCHEDULED_VISITS, patientUuid)
              }
            >
              <span>
