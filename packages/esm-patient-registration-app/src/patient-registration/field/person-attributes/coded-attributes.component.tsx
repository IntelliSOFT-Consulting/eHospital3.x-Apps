...
export const CodedAttributesField: React.FC<CodedAttributesFieldProps> = () => {
  ...
  return codedPersonAttributes?.length ? (
    <div>
      {codedPersonAttributes.map(
        (personAttributeType: CodedPersonAttributeConfig, ind) => (
...
const PersonAttributeField: React.FC<PersonAttributeFieldProps> = ({
  personAttributeTypeUuid,
  conceptUuid,
}) => {
  const { data: personAttributeType, isLoading } = usePersonAttributeType(
    personAttributeTypeUuid
  );
