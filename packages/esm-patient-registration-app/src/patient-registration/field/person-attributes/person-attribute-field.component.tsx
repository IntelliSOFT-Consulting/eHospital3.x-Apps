...
export function PersonAttributeField({
  fieldDefinition,
}: PersonAttributeFieldProps) {
  const {
    data: personAttributeType,
    isLoading,
    error,
  } = usePersonAttributeType(fieldDefinition.uuid);
    ...
    return (
      <div>
        ...
        <InlineNotification kind="error" title={t("error", "Error")}>
          {t(
            "unableToFetch",
            "Unable to fetch person attribute type - {{personattributetype}}",
            {
              personattributetype:
