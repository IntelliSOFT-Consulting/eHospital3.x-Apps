...
describe.skip("identifier input", () => {
  ...
  const setupIdentifierInput = async (
    identifierType: PatientIdentifierType
  ) => {
    initialFormValues["source-for-" + identifierType.fieldName] =
      identifierType.identifierSources[0].name;
    render(
    ...
    let identifierSourceSelectInput = screen.getByLabelText(
  ...
  it("exists", async () => {
    const { identifierInput, identifierSourceSelectInput } =
  ...
  it("has correct props for identifier source select input", async () => {
    const { identifierSourceSelectInput } = await setupIdentifierInput(
      openmrsID
    );
    expect(identifierSourceSelectInput.childElementCount).toBe(3);
    expect(identifierSourceSelectInput.value).toBe(
      "Generator 1 for OpenMRS ID"
    );
  });
  ...
  it("text input should not be disabled if manual entry is enabled", async () => {
    // setup
    openmrsID.identifierSources[0].autoGenerationOption.manualEntryEnabled =
      true;
    // replay
    const { identifierInput } = await setupIdentifierInput(openmrsID);
    expect(identifierInput.placeholder).toBe("Auto-generated");
