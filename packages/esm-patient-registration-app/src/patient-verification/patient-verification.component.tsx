...
} from "./assets/verification-assets";
import {
  searchClientRegistry,
  useGlobalProperties,
} from "./patient-verification-hook";
  ...
  if (error) {
    return (
      <Tile className={styles.errorWrapper}>
        <p>
          Error occurred while reaching the client registry, please proceed with
          registration and try again later
        </p>
      </Tile>
    );
  }
  return (
    <div id={"patientVerification"}>
      <h3 className={styles.productiveHeading02} style={{ color: "#161616" }}>
        {t(
          "clientVerificationWithClientRegistry",
      ...
      <div style={{ margin: "1rem 0 1rem" }}>
        ...
        <Tile className={styles.verificationWrapper}>
            ...
            <ComboBox
              ...
              onChange={({ selectedItem }) =>
                setVerificationCriteria({
                  ...
                  identifierType: selectedItem.value,
                })
              }
            />
          </Layer>
          ...
          {!isLoadingSearch ? (
            <Button
              disabled={!verificationCriteria.identifierType && !isLoading}
              size="md"
            ...
            <InlineLoading
              status="active"
              iconDescription="Loading"
              description="Searching client registry"
            />
          )}
        </Tile>
      </div>
