...
} from "./patient-verification-utils";
...
export function useConceptAnswers(conceptUuid: string): {
  ...
  const { data, error, isLoading } = useSWR<
    ...
    Error
  >(`/ws/rest/v1/concept/${conceptUuid}`, openmrsFetch);
  if (error) {
    showToast({
      title: error.name,
...
async function postToRegistry(
  ...
  try {
    const clientRegistryResponse = await savePatientToClientRegistry(
      formValues
    );
    if (clientRegistryResponse.ok) {
      ...
      setValues({
        ...formValues,
        identifiers: { ...formValues.identifiers, ...nupiIdentifier },
      });
      showToast({
